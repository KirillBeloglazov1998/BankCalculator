...
const InterestRate = (props) => {
    ...
    let interestRate = () => {
        ...
        if(!check.includes(false)) {
            if (( (Number.isInteger(+interestRateNew) && interestRateNew[2] !== '0') || interestRateNew[2] === '.' ) && +interestRateNew <= 30 && interestRateNew.length <= 4 ) {
                props.interestRate(interestRateNew)            
        }

        if (interestRateNew[1] === '.' && interestRateNew[2] !== '.' && interestRateNew.length <= 4 && interestRateNew[2] !== '0') { 
            props.interestRate(interestRateNew)
        }
        }
    }
